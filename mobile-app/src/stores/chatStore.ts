...
import { Message, Source, QueryContext, TimelineEvent } from '../types';
...
interface ChatState {
  ...
  toggleTimeline: () => void;
  loadTimelineEvents: (teamId: string) => Promise<void>;
}
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  sendMessage: async (text: string, teamId: string) => {
    console.log('🔥 ChatStore sendMessage called:', { text, teamId });
  ...
  setCurrentQuery: (query: string) => {
    console.log('📝 ChatStore setCurrentQuery called:', query);
  ...
  loadTimelineEvents: async (teamId: string) => {
    ...
    const mockEvents: TimelineEvent[] = [
      {
        ...
        description: 'Team decided to use Stripe for payment processing after evaluating multiple options.',
        timestamp: new Date('2024-01-15T15:45:00'),
      ...
      },
      {
        id: 'evt2',
        type: 'discussion',
        title: 'Payment Gateway Evaluation',
        description: 'Initial discussion comparing Stripe, PayPal, and Square payment solutions.',
        timestamp: new Date('2024-01-15T14:30:00'),
      ...
      },
      {
        ...
        description: 'PostgreSQL selected as primary database with Redis for caching.',
        timestamp: new Date('2024-01-14T16:20:00'),
        participants: ['Mike Chen', 'Alex Rivera'],
      ...
      },
      {
        ...
        description: 'Team discussed React vs Vue.js for the frontend application.',
        timestamp: new Date('2024-01-12T10:15:00'),
        participants: ['Alex Rivera', 'Sarah Johnson'],
        sources: [],
      },
    ];

    set({ timelineEvents: mockEvents });
  },
}));